...
const cakes = (recipe, available) => {
    const amounts = []
    // we iterate over the recipe object - ingredient is one of the keys for every 
    // iteration
    for (let ingredient in recipe) {
        // if we don't have this ingredient in the available object we can bake 0 cakes
        if (!available[ingredient]) return 0;
        // we add the rounded down result of the amount in the available list
        // divided by the value of the amount in recipe to the amounts array
        amounts.push(Math.floor(available[ingredient] / recipe[ingredient]))
    }
    // get the lowest number of the amounts array - Math.min() does not accept an 
    // array so we have to use the spread operator
